            ...
            with open(f_path, 'r', encoding='utf-8') as f:
                ...
                for row in reader:
                    if tuple(row) in seen or len(row) < 13 or row[0] != 'Transaction History' or row[1] != 'Data':
                        continue
                    seen.add(tuple(row))
                    dt, tx_type, sym, q, p, net, desc = row[2], row[5], row[6].strip(
                    ), row[7], row[8], row[12], row[4]
                    if tx_type in ('Buy', 'Sell'):
                        trades.append({'date': dt, 'symbol': sym, 'type': tx_type, 'qty': float(
                            q), 'price': float(p), 'net': float(net)})
                    elif tx_type == 'Dividend':
                        dividends.append({'date': dt, 'symbol': sym, 'type': 'dividend', 'amount': float(
                            net), 'description': desc})
                    elif tx_type == 'Foreign Tax Withholding':
                        dividends.append({'date': dt, 'symbol': sym, 'type': 'tax', 'amount': float(
                            net), 'description': desc})
        except:
            continue
...
def compute_dividends_data(entries):
    bt, bq = defaultdict(lambda: {'g': 0.0, 't': 0.0}), defaultdict(
        lambda: {'g': 0.0, 't': 0.0})
    for e in entries:
    ...
    ticker_list = []
    for s in bt:
        ticker_list.append({'ticker': s, 'name': TICKER_NAMES.get(s, s), 'net': round(
            bt[s]['g']-bt[s]['t'], 2), 'gross': round(bt[s]['g'], 2), 'tax': round(bt[s]['t'], 2)})
    return {
...
def compute_cashflow_history(trades, dividend_entries):
    ...
    monthly_buys, monthly_sells, monthly_div_net = defaultdict(
...
def replay_holdings_by_month(trades, periods):
    ...
    for period in periods:
        ...
        while trade_idx < len(sorted_trades) and sorted_trades[trade_idx]['date'] <= month_end:
            ...
            trade_idx += 1
        result[period] = {s: round(q, 4)
                          for s, q in holdings.items() if q >= 0.0001}
    return result
...
def compute_portfolio_history(trades, dividend_entries, cashflow_history):
    ...
    for cf_point in series_cf:
        ...
        if net_inv > 0:
            ...
            price_return_pct = round(
                ((market_value - net_inv) / net_inv) * 100, 2)
        result_series.append({**cf_point, 'marketValue': round(market_value, 2),
                              'totalReturnPct': total_return_pct, 'priceReturnPct': price_return_pct})
    ...
    return {'series': result_series, 'benchmark': benchmark,
...
def portfolio():
    try:
        ...
        buys_cost, buys_qty, sells_qty, first_p = defaultdict(
            float), defaultdict(float), defaultdict(float), {}
        ...
        for s in buys_qty:
            ...
            holdings.append({'ticker': s, 'name': TICKER_NAMES.get(s, s), 'qty': round(nq, 4), 'avgPrice': round(
                avg, 4), 'totalCost': round(avg * nq, 2), 'firstBuyPrice': first_p.get(s, 0)})
            tickers_to_fetch.append(s)
        prices = {}
        if tickers_to_fetch:
            try:
                data = yf.download(tickers_to_fetch, period="1d",
                                   progress=False, auto_adjust=True)
                for s in tickers_to_fetch:
                    try:
                        val = data['Close'].iloc[-1] if len(
                            tickers_to_fetch) == 1 else data['Close'][s].dropna().iloc[-1]
                        prices[s] = round(float(val), 4)
                    except:
                        prices[s] = next((h['avgPrice']
                                         for h in holdings if h['ticker'] == s), 0)
            except:
                for s in tickers_to_fetch:
                    prices[s] = next((h['avgPrice']
                                     for h in holdings if h['ticker'] == s), 0)
